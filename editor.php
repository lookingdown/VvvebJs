...
<html lang="en">
  <head>
    ...
    <link href="css/editorall.css" rel="stylesheet">
	<link href="css/givanz.css" rel="stylesheet">
    <link href="css/line-awesome.css" rel="stylesheet">
  </head>
<body>
<?php
include 'curtheme.php';
?>
	<div id="vvveb-builder">
				...
				<div id="top-panel">
					...
					<div style="padding-left:50px; padding-top: 5px;" class="btn-group mr-3 input-group-sm float-left text-center" role="group">
					  <div class="input-group-prepend">
						<label class="input-group-text" for="drop">Theme</label>
					  </div>
					  <select name="theme" id="drop"class="form-control">
						<option hidden selected>Select a theme</option>
							<?php
								$dirs = 'css/dist/'; 
								function showDir( $dir , $subdir = 0 )   {
									...
									foreach( $scan as $key => $val ) {
										...
										if ( is_dir( $dir . "/" . $val ) ) {
											...
											if(isset($themes) && $themes == $val )
											{ echo "selected >" . str_repeat( "--", $subdir ) . $val . "</option>\n";
											}else{
					...
					<input type="text" class="btn btn-light" title="Without spaces in name" name="filenames" id="filenames" placeholder="Desired PHP Filename" >
					  ...
					  <button class="btn btn-light" title="Save in builder" id="save-btn" data-vvveb-action="saveAjax" data-vvveb-shortcut="ctrl+e">
				...
				<div id="left-panel">
					  ...
					  <div id="filemanager"> 
							<div class="header">
								<a href="#" class="text-secondary">Pages: Add:</a>
									...
									<div class="btn-group responsive-btns mr-4 float-right" role="group">
									  <button class="btn btn-link btn-sm" title="New file" id="new-file-btn" data-vvveb-action="newPage" data-vvveb-shortcut="">
										  <i class="la la-file"></i><small>Page</small>
									  </button>
									  <button class="btn btn-link btn-sm"  title="Add template" id="new-template-btn" data-vvveb-action="newTemplate" data-vvveb-shortcut=""><i class="la la-plus-square"></i><small>Template</small>
									  </button> 
									</div>
  ...
  <div class="modal-dialog" role="document">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body text">
		...
		?>
		<option value="<?php echo "templates/".$pathInfo['basename'];?>"><?php echo ucfirst($pathInfo['filename']);?></option>
		<?php } ?>    
...
<!-- new template modal -->
<div class="modal fade" id="new-template-modal" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    
    <form id="templatefile">
		
    <div class="modal-content">
      <div class="modal-header">
        <p class="modal-title text-primary"><i class="la la-lg la-file"></i> New template</p>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true"><small><i class="la la-close"></i></small></span>
        </button>
      </div>
...
$(document).ready(function() 
{
	$("#drop").change(function () {
        var theme = this.value;
                  ...
                  if(data == 'true'){
                  alert('The theme has been updated to:  ' + theme);
